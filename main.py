...
def extract_details(objects, sample_size, chunk_size):
    all_details = []
    all_failed_ids = {}

    sample_size = min(sample_size, len(objects["objectIDs"]))

    object_ids = random.sample(objects["objectIDs"], sample_size)
    batch_num = 1
    ...
    for chunk_start in range(0, len(object_ids), chunk_size):
        ...
        # Repackage chunk into API format
...
def get_object_details(details_file, objects, sample_size, chunk_size):
    ...
    all_details, all_failed_ids = extract_details(objects, sample_size, chunk_size)
...
def main():
    ...
    all_details = get_object_details(details_file, objects, sample_size, chunk_size)
